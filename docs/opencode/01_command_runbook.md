...
7. Si una corrida necesita PostgreSQL, el flujo canonico es levantar `docker compose up -d db` y ejecutar la suite dentro del contenedor de backend con `docker compose run --rm backend pytest ...`; la ejecucion en host solo es fallback documentado cuando Docker no esta disponible.
8. `review-slice` acepta `BE-00X` o `FE-00X` y siempre deja `docs/opencode/reviews/BE-00X-review.md` con decision.
9. `clean-architecture-review BE-00X` siempre deja `docs/opencode/reviews/BE-00X-clean-architecture-review.md`.
10. `security-review BE-00X` siempre deja `docs/opencode/reviews/BE-00X-security-review.md`.
11. `implement-findings` acepta `BE-00X` o `FE-00X`, toma el reporte de hallazgos, el archivo de bloqueo de QA o los archivos de review del slice y cierra correcciones sobre el mismo slice vertical.
12. `/implement-findings` deja findings QA en `READY_FOR_REVALIDATION`; QA es el unico que puede declarar `RESOLVED`.
13. Las revisiones reciben un ID explicito y siempre escriben decision `APPROVED|REJECTED`.
14. `run-checks BE-00X` deja evidencia Markdown antes de `update-docs BE-00X`.
15. `final-gate BE-00X` deja un reporte final de release cuando se usa una segunda opinion de alta capacidad.

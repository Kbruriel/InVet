# FE-005 — Administración de clínica y sucursales

## Objetivo

Panel clínica para datos generales y sucursales.

## Dependencias

- Backend relacionado: `BE-005`.
- QA relacionado: `QA-005`.
- Tokens y reglas visuales en `docs/opencode/references/frontend_visual_alignment.md`.

## Alcance MVP

- Implementar UI y consumo API del slice `005`.
...
- [x] Rutas implementadas (`/clinic-administration`).
- [x] Componentes implementados (ClinicPanel, ClinicForm).
- [x] Estados UX implementados (loading/error/empty/success/submitting).
- [x] Validaciones implementadas (cliente + servidor).
- [x] Consumo API centralizado (clinic-admin-client.ts).
- [x] Sin alcance fuera del MVP.
- [x] QA aprobada (QA-005-results.md: APPROVED).
- [x] Tests frontend 13/13 PASSED.
- [x] Lint y typecheck frontend PASSED.

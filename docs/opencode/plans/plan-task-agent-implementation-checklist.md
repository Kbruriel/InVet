...
El comando `/plan-task` usa `invet-product-planner`. La actualizacion schema v2 quedo implementada: acepta IDs BE/FE/QA, genera un plan vertical con contrato frontend detallado y valida estructura, dependencias, evidencia y gates mediante `backend/scripts/validate_slice_plan.py`.

## Checklist de implementacion

- [x] 1. Objetivo: Actualizar el contrato del comando `/plan-task` para exigir un checklist numerado como salida del plan.
...
- [x] 2. Objetivo: Actualizar las responsabilidades del agente `invet-product-planner` para producir tareas atomicas.
...
- [x] 3. Objetivo: Definir reglas de paralelismo para las tareas generadas por `/plan-task`.
...
- [x] 4. Objetivo: Definir criterios de aceptacion medibles para cada tarea generada por `/plan-task`.
...
- [x] 5. Objetivo: Actualizar la documentacion operativa del contrato `/plan-task`.
...
- [x] 6. Objetivo: Sincronizar el espejo `payload` con los cambios del comando, agente y documentacion.
...
- [x] 7. Objetivo: Validar que el formato del plan generado cumpla la estructura solicitada.
...
- [x] 8. Objetivo: Documentar el cierre de la actualizacion del agente.

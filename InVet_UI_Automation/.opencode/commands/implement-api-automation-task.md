...
2.2. Inicia la fase con `python backend/scripts/manage_slice_task.py start BE-00X --task APIA-00X`, publica estados con `state` y termina con `finish --task APIA-00X --evidence "comando y resultado"` para guardar checkpoint.
...
8. Ejecuta `npm run test:api` desde `InVet_UI_Automation` cuando el entorno este listo.

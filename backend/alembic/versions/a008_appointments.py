...
def upgrade() -> None:
    ...
    appointment_type_enum = sa.Enum(
        'consultation', 'vaccination', 'surgery', 'follow_up',
        'emergency', 'other',
    ...
    op.create_table(
        ...
        sa.Column('pet_id', sa.Integer(), sa.ForeignKey('pets.id', ondelete='SET NULL'), nullable=True),
        sa.Column('veterinarian_id', sa.Integer(), sa.ForeignKey('veterinarians.id', ondelete='SET NULL'), nullable=True),
        sa.Column('clinic_id', sa.Integer(), sa.ForeignKey('clinics.id', ondelete='CASCADE'), nullable=False),
        sa.Column('branch_id', sa.Integer(), sa.ForeignKey('branches.id', ondelete='SET NULL'), nullable=False),
        sa.Column('appointment_type', appointment_type_enum, nullable=False, server_default='consultation'),
        ...
        sa.Column('created_by', sa.Integer(), sa.ForeignKey('internal_users.id', ondelete='SET NULL'), nullable=True),
        sa.Column('updated_at', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.CheckConstraint('scheduled_end > scheduled_start', name='chk_duration_positive'),
        sa.UniqueConstraint('veterinarian_id', 'scheduled_start', name='uq_vet_schedule'),
    )
    ...
    op.create_index('ix_appointments_clinic_branch_date', 'appointments', ['clinic_id', 'branch_id', 'scheduled_start'])

...
from app.api.schemas.pet_schema import Pet, PetCreate, PetUpdate, PaginatedPetsResponse
...
async def get_pets(
    ...
    pets = []
    for owner in owners:
        pets.extend(use_case.get_pets_by_owner(owner.id, 0, None))

    if breed is not None:
        pets = [pet for pet in pets if pet.breed == breed]

    pets = sorted(pets, key=lambda pet: pet.id)
    total = len(pets)
...
async def update_pet(
    ...
    owner = _get_owner_or_404(db, existing_pet.owner_id)
    _ensure_owner_access(db, current_user["id"], owner)

    payload = pet.model_dump(exclude_unset=True)
    if "owner_id" in payload:
        new_owner = _get_owner_or_404(db, payload["owner_id"])

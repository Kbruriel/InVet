...
from app.infrastructure.database.repositories.pet_repository_impl import PetRepositoryImpl
...
def get_pet_use_case(repository: PetRepository = Depends(get_pet_repository)) -> PetUseCase:
    """Inyección de dependencia para el caso de uso de mascotas."""
    return PetUseCase(repository)


@router.get("/", response_model=List[Pet])
async def get_pets(skip: int = 0, limit: int = 100, use_case: PetUseCase = Depends(get_pet_use_case)):

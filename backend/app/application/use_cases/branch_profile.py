...
from app.domain.entities.branch import AvailabilitySummary, Branch, RatingSummary
...
class GetBranchPublicProfileUseCase:
    ...
    async def execute(self, branch_id: int) -> Branch | None:
        ...
        availability_summary = (
            await self.availability_repo.get_availability_summary_by_branch(branch_id)
        )
...
class GetBranchProtectedProfileUseCase:
    ...
    async def execute(
        ...
        if not await self.branch_repo.is_branch_accessible(
            branch_id, clinic_id, current_user
        ):
            return None

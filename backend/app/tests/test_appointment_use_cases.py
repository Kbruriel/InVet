...
class MockAppointmentRepository:
    ...
    async def get_by_id(
        self, appointment_id: int, clinic_id: int | None = None
    ) -> Appointment | None:
    ...
    async def update_appointment(
        self, appointment_id: int, clinic_id: int, updates: dict
    ) -> Appointment | None:
        appt = self._items.get(appointment_id)
    ...
    async def transition_status(
        ...
        if duration_minutes is not None:
            appt.scheduled_end = appt.scheduled_start + timedelta(
    ...
    async def list_by_owner(
        ...
        page: int = 1,
        size: int = 20,
        _clinic_id: int | None = None,
    ) -> tuple[list[Appointment], int]:
        items = [
            a
            for a in self._items.values()
            if a.owner_id == owner_id and (not _clinic_id or a.clinic_id == _clinic_id)
        ]
        total = len(items)
        start = (page - 1) * size
    ...
    async def list_by_clinic(
        self, clinic_id: int, page: int = 1, size: int = 20, **filters
    ) -> tuple[list[Appointment], int]:
    ...
    async def list_by_veterinarian(
        ...
        items = [
            a
            for a in self._items.values()
            if a.veterinarian_id == veterinarian_id and a.clinic_id == clinic_id
        ]
        total = len(items)
        start = (page - 1) * size
        return items[start : start + size], total
...
class TestCreateAppointmentUseCase:
    ...
    async def test_invalid_duration_too_short(self):
        ...
        start = datetime.now(UTC) + timedelta(days=1)
        with pytest.raises(
            ValueError, match="La duración debe estar entre 15 y 120 minutos"
        ):
            await use_case.execute(
                owner_id=1,
                pet_id=10,
...
class TestTransitionAppointmentStatusUseCase:
    ...
    async def test_pending_to_approved(self):
        repo = MockAppointmentRepository()
        start = datetime.now(UTC) + timedelta(days=1)
        _appt = await repo.create_appointment(
            Appointment(
                id=1,
                owner_id=1,
                pet_id=10,
    ...
    async def test_nonexistent_appointment(self):
        ...
        with pytest.raises(ValueError, match="no encontrada"):
            await use_case.execute(
                appointment_id=999, clinic_id=1, action="approve", updated_by=None
            )
...
class TestListAppointmentsByOwnerUseCase:
        ...
        for i in range(5):
            await repo.create_appointment(
                Appointment(
                    ...
                    scheduled_end=(start + timedelta(hours=i) + timedelta(minutes=30)),
                )
            )
        from app.application.use_cases.appointment_use_cases import (
    ...
    async def test_empty_list(self):
        ...
        items, total = await use_case.execute(
            owner_id=999, clinic_id=1, page=1, size=20
        )
        assert total == 0
        assert len(items) == 0

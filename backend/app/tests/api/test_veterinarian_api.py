...
from app.tests.integration.test_veterinarian_crud import VeterinarianRepositoryImpl
from app.domain.entities.veterinarian import VeterinarianCreate
...
async def test_create_veterinarian():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        response = await client.post("/api/v1/veterinarians/", json={
            "branch_id": 1,
            "name": "Carlos",
            "last_name": "García",
            "specialty": "Cirugía",
...
async def test_get_veterinarian():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        create_response = await client.post("/api/v1/veterinarians/", json={
            "branch_id": 1,
            "name": "María",
            "last_name": "Rodríguez",
...
async def test_update_veterinarian():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        # Then update it
        update_response = await client.put(f"/api/v1/veterinarians/{vet_id}", json={
            "name": "Juan Carlos",
            "last_name": "Pérez",

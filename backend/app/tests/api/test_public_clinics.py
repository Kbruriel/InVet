...
def mock_use_case():
    ...
    use_case.execute = AsyncMock(
        return_value=PublicClinicsPaginatedResponse(
            data=[
                PublicClinicListDTO(
                    id=1, name="Clínica Test", description="Descripción de prueba",
                    city="Ciudad Test", address="Calle Test 123", rating=None,
                )
            ],
    ...
    use_case.get_by_id = AsyncMock(
        return_value=PublicClinicDetailDTO(
            id=1, name="Clínica Test", description="Descripción de prueba",
            city="Ciudad Test", address="Calle Test 123", rating=None,
            state="Estado Test", country="País Test", phone="1234567890",
        )
    )
    return use_case
...
async def async_app(mock_use_case):
    ...
    app.dependency_overrides[pub_clinics_mod.get_public_clinic_list_use_case] = mock_get_use_case

...
from app.infrastructure.database.models.appointment import AppointmentSlot as AppointmentSlotDB
from app.domain.entities.appointment import Appointment, AppointmentCreate, AppointmentSlot, AppointmentSlotCreate
from app.domain.repositories.appointment_repo import AppointmentRepository, AppointmentSlotRepository
...
class AppointmentRepositoryImpl(AppointmentRepository):
    ...
    def find_by_id(self, appointment_id: int) -> Optional[Appointment]:
        db_appointment = self.db.query(AppointmentDB).filter(AppointmentDB.id == appointment_id).first()
    ...
    def find_by_owner_id(self, owner_id: int) -> List[Appointment]:
        db_appointments = self.db.query(AppointmentDB).filter(AppointmentDB.owner_id == owner_id).all()
        return [
            Appointment(
                id=db_appointment.id,
                owner_id=db_appointment.owner_id,
    ...
    def reschedule(self, appointment_id: int, new_slot_id: int) -> Optional[Appointment]:
        db_appointment = self.db.query(AppointmentDB).filter(AppointmentDB.id == appointment_id).first()
        if not db_appointment:
            return None
...
class AppointmentSlotRepositoryImpl(AppointmentSlotRepository):
    ...
    def find_available_slots_by_clinic_branch(
        ...
        db_slots = self.db.query(AppointmentSlotDB).filter(
    ...
    def find_by_id(self, slot_id: int) -> Optional[AppointmentSlot]:
        db_slot = self.db.query(AppointmentSlotDB).filter(AppointmentSlotDB.id == slot_id).first()
        if not db_slot:
            return None
    ...
    def _set_slot_availability(self, slot_id: int, is_available: bool) -> Optional[AppointmentSlot]:
        db_slot = self.db.query(AppointmentSlotDB).filter(AppointmentSlotDB.id == slot_id).first()
        if not db_slot:
            return None

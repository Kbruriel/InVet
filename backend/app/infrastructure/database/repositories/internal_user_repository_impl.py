...
from app.infrastructure.database.models.internal_user import InternalUser as InternalUserDB


class InternalUserRepositoryImpl:
    """Implementación del repositorio de usuarios internos"""
    ...
    def create_internal_user(self, user_data: InternalUserCreate) -> InternalUser:
        ...
        db_user = InternalUserDB(**user_data.model_dump())
        self.db.add(db_user)
        self.db.commit()
    ...
    def get_internal_user(self, user_id: int) -> Optional[InternalUser]:
        ...
        db_user = self.db.query(InternalUserDB).filter(InternalUserDB.id == user_id).first()

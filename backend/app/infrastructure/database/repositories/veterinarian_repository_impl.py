...
from app.domain.entities.veterinarian import Veterinarian, VeterinarianCreate, VeterinarianUpdate
from app.infrastructure.database.models.veterinarian import Veterinarian as VeterinarianDB


class VeterinarianRepositoryImpl:
    """Implementación del repositorio de veterinarios"""

    def __init__(self, db: Session):
        self.db = db

    def create_veterinarian(self, veterinarian_data: VeterinarianCreate) -> Veterinarian:
        """Crea un nuevo veterinario"""
        db_veterinarian = VeterinarianDB(**veterinarian_data.model_dump())
    ...
    def get_veterinarian(self, veterinarian_id: int) -> Optional[Veterinarian]:
        ...
        db_veterinarian = self.db.query(VeterinarianDB).filter(VeterinarianDB.id == veterinarian_id).first()
        return self._db_to_domain(db_veterinarian) if db_veterinarian else None

    def get_veterinarians(self, branch_id: int, skip: int = 0, limit: int = 100) -> List[Veterinarian]:
        ...
        return [self._db_to_domain(db_veterinarian) for db_veterinarian in db_veterinarians]

    def update_veterinarian(self, veterinarian_id: int, veterinarian_data: VeterinarianUpdate) -> Optional[Veterinarian]:
        ...
        # Update fields
        for key, value in veterinarian_data.model_dump(exclude_unset=True).items():
    ...
    def _db_to_domain(self, db_veterinarian: VeterinarianDB) -> Veterinarian:
        ...
        return Veterinarian(
            ...
            name=db_veterinarian.first_name,  # Fixed: mapping first_name from DB to name in domain

...
import { ErrorBanner, EmptyState } from '@/shared/ui'
...
export default async function ClinicBranchPage({ params }: Params) {
  ...
  const branchId = parseInt(clinicId, 10)

  if (Number.isNaN(branchId)) {
    return <ErrorBanner title="Datos invalidos" message="El ID de sucursal no es valido." />
  }
  ...
  try {
    ...
    return (
      <ErrorBanner
        ...
        message="No se pudieron cargar los datos de la sucursal. Intente de nuevo mas tarde."
      />
    )
  }
}
